...
class AnalyticsActivity : AppCompatActivity() { // we created the class to handle the financial data visualization
    ...
    private fun setupCategoryList() { // We set up the RecyclerView with a linear layout manager
        ...
        db.getAllExpenses { expensesList ->
            val categoryTotalsMap: Map<String, Double> = expensesList
                .groupBy { it.category }
                .mapValues { entry ->  entry.value.sumOf { it.amount } }

            val adapter = CategoryAdapter(categoryTotalsMap) // We bind the database results to the RecyclerView using our custom adapter
            rvCategories.adapter = adapter

            Log.d("SmartSpend", "Category totals fetched from DB: ${categoryTotalsMap.size} items found")
        }
